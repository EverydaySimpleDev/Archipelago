...
WORLDS = ["Destiny Islands", "Traverse Town", "Wonderland", "Olympus Coliseum", "Deep Jungle", "Agrabah", "Monstro",
          "Atlantica", "Halloween Town", "Neverland", "Hollow Bastion", "End of the World", "100 Acre Wood"]
KEYBLADES = ["Oathkeeper", "Lionheart", "Lady Luck", "Olympia", "Jungle King", "Three Wishes", "Wishing Star",
...
def has_x_worlds(state: CollectionState, player: int, num_of_worlds: int, keyblades_unlock_chests: bool,
...
def has_emblems(state: CollectionState, player: int, keyblades_unlock_chests: bool, logic_difficulty: int,
...
def has_all_arts(state: CollectionState, player: int) -> bool:
    return state.has_all(
        {"Fire Arts", "Blizzard Arts", "Thunder Arts", "Cure Arts", "Gravity Arts", "Stop Arts", "Aero Arts"}, player)
...
def has_offensive_magic(state: CollectionState, player: int, logic_difficulty: int) -> bool:
    return (
            ...
            or (logic_difficulty > LOGIC_NORMAL and state.has_any({"Progressive Thunder", "Progressive Gravity"},
                                                                  player))
...
def has_final_rest_door(state: CollectionState, player: int, final_rest_door_requirement: str,
                        final_rest_door_required_lucky_emblems: int):
...
def has_oogie_manor(state: CollectionState, player: int, logic_difficulty: int) -> bool:
    return (
            ...
            or (logic_difficulty > LOGIC_NORMAL and state.has("High Jump", player, 2) or (
...
def has_key_item(state: CollectionState, player: int, key_item: str, stacking_world_items: bool,
                 halloween_town_key_item_bundle: bool, difficulty: int, keyblades_unlock_chests: bool):
    return (
            (
                    state.has(key_item, player)
                    or (stacking_world_items and state.has(WORLD_KEY_ITEMS[key_item], player, 2))
                    or (key_item == "Jack-In-The-Box" and state.has("Forget-Me-Not",
                                                                    player) and halloween_town_key_item_bundle)
            )
            ...
            and (
                        key_item != "Crystal Trident" or difficulty > LOGIC_BEGINNER or not keyblades_unlock_chests or state.has(
                    "Crabclaw", player))
    )
...
def set_rules(kh1world):
    ...
    add_rule(kh1world.get_location("Traverse Town 1st District Accessory Shop Roof Chest"),
             # this check could justifiably require high jump for Beginners
    ...
    add_rule(kh1world.get_location("Traverse Town Geppetto's House Chest"),
             lambda state: (has_parasite_cage(state, player, difficulty,
                                              has_x_worlds(state, player, 3, options.keyblades_unlock_chests,
                                                           difficulty, hundred_acre_wood))))
                     ...
                     (
                             ...
                             (
                                     ...
                                     (
                                             ...
                                             or state.has("Combo Master", player) and (
                                                         state.has("High Jump", player) or state.has("Air Combo Plus",
                                                                                                     player, 2))
                                     )
                             )
                     )
    ...
    add_rule(kh1world.get_location("Wonderland Rabbit Hole Defeat Heartless 3 Chest"),
             lambda state: (
                     ...
                     or (difficulty > LOGIC_NORMAL and has_x_worlds(state, player, 3, options.keyblades_unlock_chests,
                                                                    difficulty, hundred_acre_wood))
    ...
    add_rule(kh1world.get_location("Wonderland Queen's Castle Hedge Left Red Chest"),
             lambda state: (
                     has_key_item(state, player, "Footprints", stacking_world_items, halloween_town_key_item_bundle,
                                  difficulty, options.keyblades_unlock_chests)
    ...
    add_rule(kh1world.get_location("Wonderland Lotus Forest Glide Chest"),
             lambda state: (
                     ...
                     (
                             ...
                             and has_key_item(state, player, "Footprints", stacking_world_items,
                                              halloween_town_key_item_bundle, difficulty,
                                              options.keyblades_unlock_chests)
                     )
                     or
                     (
                             difficulty > LOGIC_PROUD
    ...
    add_rule(kh1world.get_location("Wonderland Bizarre Room Lamp Chest"),
             lambda state: has_key_item(state, player, "Footprints", stacking_world_items,
                                        halloween_town_key_item_bundle, difficulty, options.keyblades_unlock_chests))
             ...
             lambda state: (
                     ...
                     (
                             ...
                             (
                                     ...
                                     or (state.has("High Jump", player, 2) and has_key_item(state, player, "Footprints",
    ...
    add_rule(kh1world.get_location("Halloween Town Moonlight Hill White Trinity Chest"),
             lambda state: (
                     state.has("White Trinity", player)
                     and has_key_item(state, player, "Forget-Me-Not", stacking_world_items,
                                      halloween_town_key_item_bundle, difficulty, options.keyblades_unlock_chests)
             ))
    add_rule(kh1world.get_location("Halloween Town Bridge Under Bridge"),
             lambda state: (
                     has_key_item(state, player, "Forget-Me-Not", stacking_world_items, halloween_town_key_item_bundle,
                                  difficulty, options.keyblades_unlock_chests) and has_key_item(state, player,
                                                                                                "Jack-In-The-Box",
                                                                                                stacking_world_items,
                                                                                                halloween_town_key_item_bundle,
                                                                                                difficulty,
    ...
    add_rule(kh1world.get_location("Halloween Town Boneyard Tombstone Puzzle Chest"),
             lambda state: has_key_item(state, player, "Forget-Me-Not", stacking_world_items,
                                        halloween_town_key_item_bundle, difficulty, options.keyblades_unlock_chests))
             ...
             lambda state: (
                     has_key_item(state, player, "Forget-Me-Not", stacking_world_items, halloween_town_key_item_bundle,
                                  difficulty, options.keyblades_unlock_chests) and has_key_item(state, player,
                                                                                                ...
                                                                                                options.keyblades_unlock_chests)
                     and has_oogie_manor(state, player, difficulty)
                     and (difficulty > LOGIC_BEGINNER or has_basic_tools or state.has_all(
                 {"High Jump", "Progressive Glide"}))
             ))
    ...
    add_rule(kh1world.get_location("Halloween Town Guillotine Square High Tower Chest"),
             lambda state: (
                     state.has_all({"High Jump", "Progressive Glide"}, player)
                     or (difficulty > LOGIC_BEGINNER and (
    ...
    add_rule(kh1world.get_location("Monstro Mouth Near Ship Chest"),
             lambda state: (difficulty > LOGIC_BEGINNER or state.has_any({"High Jump", "Progressive Glide"},
                                                                         player) or has_basic_tools))
    ...
    add_rule(kh1world.get_location("Hollow Bastion Rising Falls Floating Platform Near Save Chest"),
             # might be possible with CM and 2ACP
             lambda state: (
                     state.has("High Jump", player)
                     or state.has("Progressive Glide", player)
    ...
    add_rule(kh1world.get_location("Hollow Bastion Rising Falls High Platform Chest"),
             lambda state: (
                     state.has("Progressive Glide", player)
                     or (state.has("Progressive Blizzard", player) and has_emblems(state, player,
                                                                                   options.keyblades_unlock_chests,
                                                                                   difficulty, hundred_acre_wood))
                     or (difficulty > LOGIC_BEGINNER and state.has("High Jump", player, 3))
                     or (difficulty > LOGIC_NORMAL and (
                         state.has("High Jump", player) or state.has("Combo Master", player)))
                     or difficulty > LOGIC_PROUD
             ))
    add_rule(kh1world.get_location("Hollow Bastion Castle Gates Gravity Chest"),
             lambda state: (
                     ...
                     (
                             ...
                             or (difficulty > LOGIC_BEGINNER and state.has("High Jump", player, 3) and state.has(
                         "Progressive Glide", player))
                             or (difficulty > LOGIC_NORMAL and (
                                 state.has("High Jump", player, 2) or can_dumbo_skip(state, player)) and state.has(
                         "Progressive Glide", player))
    ...
    add_rule(kh1world.get_location("Hollow Bastion Castle Gates Freestanding Pillar Chest"),
             lambda state: (
                     ...
                     or (difficulty > LOGIC_BEGINNER and state.has("High Jump", player, 3))
                     or (difficulty > LOGIC_NORMAL and (
                         state.has("High Jump", player, 2) or can_dumbo_skip(state, player)))
                     or (difficulty > LOGIC_PROUD and state.has_all({"High Jump", "Progressive Glide"}, player))
             ))
    ...
    add_rule(kh1world.get_location("Hollow Bastion Entrance Hall Left of Emblem Door Chest"),
             lambda state: (
                     ...
                     (
                             ...
                             and has_emblems(state, player, options.keyblades_unlock_chests, difficulty,
                                             hundred_acre_wood)
                     )
             ))
    ...
    add_rule(kh1world.get_location("Olympus Coliseum Defeat Cerberus Inferno Band Event"),
             lambda state: has_key_item(state, player, "Entry Pass", stacking_world_items,
                                        halloween_town_key_item_bundle, difficulty, options.keyblades_unlock_chests))
    ...
    add_rule(kh1world.get_location("Traverse Town Geppetto's House Geppetto Reward 1"),
             lambda state: has_parasite_cage(state, player, difficulty,
                                             has_x_worlds(state, player, 3, options.keyblades_unlock_chests, difficulty,
                                                          hundred_acre_wood)))
    ...
    add_rule(kh1world.get_location("Traverse Town Geppetto's House Geppetto All Summons Reward"),
             lambda state:
             has_parasite_cage(state, player, difficulty,
                               has_x_worlds(state, player, 3, options.keyblades_unlock_chests, difficulty,
                                            hundred_acre_wood)
    ...
    add_rule(kh1world.get_location("Traverse Town Magician's Study Obtained All Arts Items"),
             lambda state: (
                     ...
                     and has_x_worlds(state, player, 8, options.keyblades_unlock_chests, 0, hundred_acre_wood)
             # due to the softlock potential, I'm forcing it to logic normally instead of allowing the bypass
             ))
    ...
    add_rule(kh1world.get_location("Hollow Bastion Entrance Hall Emblem Piece (Flame)"),
             lambda state: (
                     (
                             has_key_item(state, player, "Theon Vol. 6", stacking_world_items,
                                          halloween_town_key_item_bundle, difficulty, options.keyblades_unlock_chests)
    ...
    add_rule(kh1world.get_location("Monstro Throat Blue Trinity"),
             lambda state: (
                     state.has("Blue Trinity", player)
                     and has_parasite_cage(state, player, difficulty,
                                           has_x_worlds(state, player, 3, options.keyblades_unlock_chests, difficulty,
                                                        hundred_acre_wood))
             ))
        ...
        add_rule(kh1world.get_location("100 Acre Wood Bouncing Spot Left Cliff Chest"),
                 lambda state: (
                         ...
                         (
                                 state.has_all({"High Jump", "Progressive Glide"}, player)
                                 or (difficulty > LOGIC_BEGINNER and (
                                     state.has("Progressive Glide", player) or state.has("High Jump", player)))
                                 or difficulty > LOGIC_NORMAL
                         )
                 ))
            ...
            add_rule(kh1world.get_location("Olympus Coliseum Defeat Hades Ansem's Report 8"),
                     lambda state: (
                             state.has_all({
                                 ...
                                 "Pegasus Cup",
                                 "Hercules Cup"}, player)
                             and has_key_item(state, player, "Entry Pass", stacking_world_items,
                                              halloween_town_key_item_bundle, difficulty,
                                              options.keyblades_unlock_chests)
        ...
        add_rule(kh1world.get_location("Neverland Defeat Phantom Stop Event"),
                 lambda state: (
                         ...
                         (
                                 ...
                                 or (difficulty > LOGIC_NORMAL and state.has_all(
                             {"Progressive Fire", "Progressive Blizzard", "Progressive Thunder", "Progressive Stop"},
                             player))
                                 or
                                 (
                                         difficulty > LOGIC_PROUD
        ...
        add_rule(kh1world.get_location("Agrabah Defeat Kurt Zisa Ansem's Report 11"),
                 lambda state: (
                         ...
                         (
                                 ...
                                 or (difficulty > LOGIC_BEGINNER and state.has_any_count(
                             {"Progressive Blizzard": 2, "Progressive Fire": 3, "Progressive Thunder": 3,
                              "Progressive Gravity": 3}, player))
                                 or (difficulty > LOGIC_NORMAL and (state.has_any_count(
                             {"Progressive Blizzard": 1, "Progressive Fire": 2, "Progressive Thunder": 2,
                              "Progressive Gravity": 2}, player)))
                                 or (difficulty > LOGIC_PROUD and (
                                     state.has_any({"Progressive Fire", "Progressive Thunder", "Progressive Gravity"},
                                                   player) or (state.has_group("Magic", player) and state.has_all(
                                 {"Mushu", "Genie", "Dumbo"}, player))))
                         )
                 ))
    ...
    if options.jungle_slider:
        add_rule(kh1world.get_location("Deep Jungle Jungle Slider 10 Fruits"),
                 lambda state: has_key_item(state, player, "Slides", stacking_world_items,
                                            halloween_town_key_item_bundle, difficulty,
                                            options.keyblades_unlock_chests))
    ...
    add_rule(kh1world.get_location("Final Ansem"),
             lambda state: (
                     has_x_worlds(state, player, 8, options.keyblades_unlock_chests, difficulty,
                                  hundred_acre_wood)  # In logic, player is strong enough
                     and
                     (
                             (  # Can DI Finish
                             ...
                             (
                                     ...
                                     and has_final_rest_door(state, player, final_rest_door_requirement,
                                                             final_rest_door_required_lucky_emblems)
                             # Can open the Door
                             )
                     )
                     and has_defensive_tools(state, player, difficulty)
    ...
    for location in location_table.keys():
        ...
        if options.remote_items.current_key == "off":
            ...
            if location_table[location].type == "Level Slot 1":
                add_item_rule(kh1world.get_location(location),
                              lambda i: (i.player != player or item_table[i.name].category in ["Level Up",
                                                                                               "Limited Level Up"]))
            if location_table[location].type == "Level Slot 2":
                add_item_rule(kh1world.get_location(location),
                              lambda i: (i.player != player or (
                                          item_table[i.name].category in ["Level Up", "Limited Level Up"] or item_table[
                                      i.name].type == "Ability")))
        ...
        if options.keyblades_unlock_chests:
            if location_table[location].type == "Chest" or location in BROKEN_KEYBLADE_LOCKING_LOCATIONS:
                ...
                if location not in BROKEN_KEYBLADE_LOCKING_LOCATIONS:
                    add_rule(kh1world.get_location(location),
                             lambda state, location_required_keyblade=location_required_keyblade: state.has(
                                 location_required_keyblade, player))
                else:
                    add_rule(kh1world.get_location(location),
                             lambda state, location_required_keyblade=location_required_keyblade: state.has(
                                 location_required_keyblade, player) or difficulty > LOGIC_BEGINNER)
    ...
    add_rule(kh1world.get_entrance("Wonderland"),
             lambda state: state.has("Wonderland", player) and has_x_worlds(state, player, 3,
                                                                            options.keyblades_unlock_chests, difficulty,
                                                                            hundred_acre_wood))
    add_rule(kh1world.get_entrance("Olympus Coliseum"),
             lambda state: state.has("Olympus Coliseum", player) and has_x_worlds(state, player, 3,
                                                                                  options.keyblades_unlock_chests,
                                                                                  difficulty, hundred_acre_wood))
    add_rule(kh1world.get_entrance("Deep Jungle"),
             lambda state: state.has("Deep Jungle", player) and has_x_worlds(state, player, 3,
                                                                             options.keyblades_unlock_chests,
                                                                             difficulty, hundred_acre_wood))
    add_rule(kh1world.get_entrance("Agrabah"),
             lambda state: state.has("Agrabah", player) and has_x_worlds(state, player, 3,
                                                                         options.keyblades_unlock_chests, difficulty,
                                                                         hundred_acre_wood))
    add_rule(kh1world.get_entrance("Monstro"),
             lambda state: state.has("Monstro", player) and has_x_worlds(state, player, 3,
                                                                         options.keyblades_unlock_chests, difficulty,
                                                                         hundred_acre_wood))
    if options.atlantica:
        add_rule(kh1world.get_entrance("Atlantica"),
                 lambda state: state.has("Atlantica", player) and has_x_worlds(state, player, 3,
                                                                               options.keyblades_unlock_chests,
                                                                               difficulty, hundred_acre_wood))
    add_rule(kh1world.get_entrance("Halloween Town"),
             lambda state: state.has("Halloween Town", player) and has_x_worlds(state, player, 3,
                                                                                options.keyblades_unlock_chests,
                                                                                difficulty, hundred_acre_wood))
    add_rule(kh1world.get_entrance("Neverland"),
             lambda state: state.has("Neverland", player) and has_x_worlds(state, player, 4,
                                                                           options.keyblades_unlock_chests, difficulty,
                                                                           hundred_acre_wood))
    add_rule(kh1world.get_entrance("Hollow Bastion"),
             lambda state: state.has("Hollow Bastion", player) and has_x_worlds(state, player, 6,
                                                                                options.keyblades_unlock_chests,
                                                                                difficulty, hundred_acre_wood))
    add_rule(kh1world.get_entrance("End of the World"),
             lambda state: has_x_worlds(state, player, 8, options.keyblades_unlock_chests, difficulty,
                                        hundred_acre_wood) and ((has_lucky_emblems(state, player,
                                                                                   eotw_required_lucky_emblems) and end_of_the_world_unlock == "lucky_emblems") or state.has(
                 "End of the World", player)))
